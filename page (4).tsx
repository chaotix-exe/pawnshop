...
import { createEmployee, resetPassword, deleteEmployee } from "@/app/actions/auth";
...
function Logins({ users, onChange }: { users: any[]; onChange: () => void }) {
  ...
  return (
    <div>
      <div className="panel">
        <h3 style={{ color: "#f0c43c", fontSize: 14 }}>Nieuwe medewerker</h3>
        <form action={add}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            <div style={{ width: 140 }}><label>Rol</label><select name="rol"><option value="medewerker">medewerker</option><option value="admin">admin</option></select></div>
          </div>
          <div style={{ marginTop: 10 }}><button className="btn sm">+ Aanmaken</button></div>
        </form>
      </div>
      <div className="panel">
        <table><thead><tr><th>Gebruikersnaam</th><th>Naam</th><th>Rol</th><th /></tr></thead>
          <tbody>{users.map(u => <tr key={u.id}><td>{u.username}</td><td>{u.naam}</td><td>{u.rol}</td>
